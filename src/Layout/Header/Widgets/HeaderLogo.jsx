  ...
  return (
    <>
      <Link href="/" className={`${extraClass ? extraClass : ""}`}>
        {logo?.original_url ? <Image className="img-fluid" src={logo?.original_url} height={34} width={173} alt={settingData?.general?.site_name ? settingData?.general?.site_name : 'wkart-logo'} /> : settingData?.general?.site_name && <h2 className="f-w-600 m-0">{settingData?.general?.site_name ? settingData?.general?.site_name.split(" ")[0] : "Logo Here"}</h2>}
      </Link>
    </>
  );
};
