...
const FooterLogo = () => {
  ...
  return (
    <div className="footer-logo">
      <Link href="/">{logoAbc?.original_url ? <Image src={logoAbc?.original_url} height={34} width={180} alt={settingData?.general?.site_name ? settingData?.general?.site_name : "wkart-logo"} /> : settingData?.general?.site_name && <h2 className="f-w-600">{settingData?.general?.site_name ? settingData?.general?.site_name.split(" ")[0] : "Logo Here"}</h2>}</Link>
    </div>
  );
};
